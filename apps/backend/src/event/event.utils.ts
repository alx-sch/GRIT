...
/**
 * Encodes a cursor for user events pagination with support for different sort modes
 */
export function userEventEncodeCursor(
  startAt: Date,
  id: number,
  isPublished: boolean,
  sort: string
): string {
  const str = `${startAt.toISOString()}|${String(id)}|${String(isPublished)}|${sort}`;
  return Buffer.from(str).toString('base64');
}

/**
 * Decodes cursor for user events pagination with support for different sort modes
 */
export function userEventCursorFilter(input: {
  cursor?: string;
  sort?: string;
  ...
  try {
    const [startAtStr, idStr, isPublishedStr, cursorSort] = Buffer.from(input.cursor, 'base64')
      .toString('utf-8')
      .split('|');
    ...
    const sort = input.sort ?? cursorSort;

    // Build cursor filter based on sort mode
    if (sort === 'furthest') {
      // DESC order: startAt DESC, id DESC
      return {
        OR: [{ startAt: { lt: startAt } }, { startAt, id: { lt: id } }],
      };
    } else if (sort === 'soonest') {
      // ASC order: startAt ASC, id ASC
      return {
        OR: [{ startAt: { gt: startAt } }, { startAt, id: { gt: id } }],
      };
    } else {
      // drafts-first: isPublished ASC (false before true), then startAt ASC, then id ASC
      if (!isPublished) {
        // If cursor is on a draft (false), return:
        // - All published events (since they come after drafts in ASC order), OR
        // - Drafts with later startAt, OR
        // - Drafts with same startAt but higher id
        return {
          OR: [
            { isPublished: true },
            { isPublished: false, startAt: { gt: startAt } },
            { isPublished: false, startAt, id: { gt: id } },
          ],
        };
      } else {
        // If cursor is on a published event (true), return only published events after it
        return {
          OR: [
            { isPublished: true, startAt: { gt: startAt } },
            { isPublished: true, startAt, id: { gt: id } },
          ],
        };
      }
    }
