...
import {eventCursorFilter, eventEncodeCursor, eventGenerateSlug, eventSearchFilter,} from '@/event/event.utils';
...
export class EventService {
  ...
  private async resolveEventId(idOrSlug: string): Promise<number> {
    const event = await this.prisma.event.findFirst({
      where: {
        OR: [
          {slug: idOrSlug},
          {id: isNaN(Number(idOrSlug)) ? undefined : Number(idOrSlug)}
        ],
      },
      select: {id: true},
    });
  ...
  async eventDelete(idOrSlug: string, userId: number) {
    ...
    try {
      ...
      throw new UnauthorizedException(
          `No permission to delete event with id ${idOrSlug}.`);
    }
  }
  ...
  async eventGetPublished(input: ReqEventGetPublishedDto, userId?: number) {
    ...
    const orderByMap: Record<string, object[]> = {
      ...
      popularity:
          [{attendees: {_count: 'desc'}}, {startAt: 'asc'}, {id: 'asc'}],
    };
  ...
  async eventGetById(idOrSlug: string, userId?: number) {
    ...
    if (!event_raw ||
        (!event_raw.isPublished && event_raw.authorId !== userId)) {
  ...
  async eventPatch(idOrSlug: string, data: ReqEventPatchDto, userId: number) {
    ...
    if (data.title !== undefined)
      newData.title = data.title;  // Don't update slug
  ...
  async eventUpdateImage(
      idOrSlug: string, userId: number, file: Express.Multer.File) {
    const eventId = await this.resolveEventId(idOrSlug);
