...
export function mapConversationToCard(
    ...
    conversationState: ResConversationState, currentUser: CurrentUser) {
  // Set other user if it is a 1:1 conversation
  const otherUser = conversation.type === 'DIRECT' ?
      conversation.participants.find((el) => el.user.id !== currentUser.id)
          ?.user :
      undefined;
  ...
  const findImageUrl = () => {
    if (conversation.type === 'DIRECT')
      return otherUser?.avatarKey ? getAvatarImageUrl(otherUser.avatarKey) :
                                    undefined;
  ...
  const getImageFallback = () => {
    if (conversation.type === 'DIRECT')
      return otherUser?.name?.trim().slice(0, 2).toUpperCase();
    if (conversation.type === 'EVENT')
  ...
  const findTitle = () => {
    if (conversation.type === 'DIRECT')
      ...
      return conversation.event.title ?? 'Event';
    else
      return 'Conversation';
  };
  ...
  // Last Message Text
  const lastMessageTextLong =
      conversationState?.lastMessage?.text ?? 'No messages yet';
  ...
  if (conversationState?.lastMessage?.author?.id === currentUser.id)
    ...
    lastMessageAuthorNameLong = conversationState?.lastMessage?.author.name;
  ...
  // Otherwise we must compage
    ...
    hasUnread = new Date(lastMessage.createdAt).getTime() >
