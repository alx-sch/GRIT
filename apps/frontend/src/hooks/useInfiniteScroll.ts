 ...
 * Creates the loadMore function and sets up IntersectionObserver for events (auto-load).
 * For locations, loadMore is called manually via scroll listeners.
 * Returns items, pagination, loadMore function, and sentinel ref for the component to use.
 */
export function useInfiniteScroll<T extends PaginatedItem>(
  ...
  fetchMore: (cursor: string) => Promise<{ data: T[]; pagination: Pagination }>, // A function parameter (function that fetches next page of items using cursor).
  dependencies: unknown[] = [] // External values that trigger observer recreation (e.g., searchParams, filters)
) {
  ...
  const [isLoading, setIsLoading] = useState(false); // Avoids duplicate requests if one request is currently happening
  const sentinelRef = useRef<HTMLDivElement | null>(null); // Reference to sentinel div (IntersectionObserver watches this)

  // Main function for loading more items -> takes whichever items were already there, and appends the next ones to also be displayed.
  ...
  return { items, isLoading, sentinelRef, pagination, loadMore }; // Returns an object
}
