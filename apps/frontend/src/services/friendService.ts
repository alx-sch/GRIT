...
import type {FriendBase, FriendRequestBase, FriendRequestResponse, FriendResponse} from '@/types/friends';
...
export const friendService = {
  ...
  removeFriend: async(friendId: number): Promise<FriendBase> => {
    const response =
        await api.delete<FriendBase>(`/users/me/friends/${friendId}`);
    return response.data;
  },
  ...
  listFriends: async(params?: getFriendsParams): Promise<FriendResponse> => {
    ...
    const url =
        queryString ? `/users/me/friends?${queryString}` : '/users/me/friends';
    const response = await api.get<FriendResponse>(url);
    return response.data;
  },

  listIncomingRequests: async(params?: getFriendRequestsParams):
  ...
  listOutgoingRequests: async(params?: getFriendRequestsParams):
      Promise<FriendRequestResponse> => {
        const queryParams = new URLSearchParams();
        if (params?.limit) queryParams.set('limit', params.limit);
        if (params?.cursor) queryParams.set('cursor', params.cursor);
