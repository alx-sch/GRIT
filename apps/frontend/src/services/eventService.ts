...
export const eventService = {
  ...
  uploadEventImage: async(
      ...
      Promise<EventBase> => {
        ...
        const response = await api.patch<EventBase>(
            `/events/${String(eventId)}/upload-image`, formData, {
  ...
  deleteEventImage: async(eventId: number): Promise<EventBase> => {
    const response =
        await api.delete<EventBase>(`/events/${String(eventId)}/image`);
    return response.data;
  },
  ...
  uploadEventFile: async(
      ...
      Promise<EventBase> => {
        ...
        const response = await api.post<EventBase>(
            `/events/${String(eventId)}/files`, formData, {
              headers: {'Content-Type': 'multipart/form-data'},
              onUploadProgress: (e) =>
                  onProgress?.(Math.round((e.loaded * 100) / (e.total ?? 1))),
            });
  ...
  deleteEventFile: async(eventId: number, fileId: number):
      Promise<EventBase> => {
