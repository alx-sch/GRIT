...
// Setup logging via Fastify's logger
...
if (fs.existsSync(schemaPath)) {
  ...
  db.exec(schemaSQL);
  fastify.log.info('Dev database created with schema.');
} else {
  fastify.log.warn('schema.sql not found, database tables not created.');
}
...
if (userCount === 0) {
  ...
  if (fs.existsSync(seedPath)) {
    ...
    db.exec(seedSQL);
    fastify.log.info('Demo users seeded.');
  } else {
